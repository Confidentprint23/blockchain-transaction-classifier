...
def load_all_data():
    ...
    try:
        ...
        if not os.path.exists('data'):
            ...
            st.info("📁 Please create a 'data' folder and add your exported files.")
            st.code("""
...
└── requirements.txt
        ...
        st.info("💡 Make sure all required files are in the 'data' folder")
        st.code("""
...
9. feature_cols.pkl
        ...
        st.info("💡 Check if all pickle files were exported correctly from Colab")
        return None
...
st.markdown('<div class="main-header">🔗 Blockchain Transaction Security Dashboard</div>', 
            unsafe_allow_html=True)
...
st.sidebar.markdown("---")
st.sidebar.info("💡 **Tip**: Select different views to explore various aspects of the analysis.")
...
if page == "🏠 Overview":
    ...
    with col1:
        ...
        st.info(f"📈 **Precision**: {precision_score(y_test, evaluation_results.get('predictions', [])):.3f}")
        st.info(f"📈 **Recall**: {recall_score(y_test, evaluation_results.get('predictions', [])):.3f}")
    ...
    with col2:
        ...
        st.info(f"🛡️ **BFT Capacity**: {bft_metrics.get('max_tolerable_bft', 0)} nodes")
...
# ============================================
    ...
    st.info(f"📊 Showing {len(filtered_anomalies)} anomalies matching filters")
    ...
    if high_risk_anomalies:
        ...
        st.success("✅ No high-risk anomalies detected with current filters")

# ============================================
# PAGE 4: BLOCKCHAIN ANALYSIS
# ============================================

elif page == "🔗 Blockchain Analysis":
    
    st.header("🔗 Blockchain Network & Economics Analysis")
    
    tabs = st.tabs(["🕸️ Network Topology", "💰 Economics", "🔐 Security"])
    
    with tabs[0]:
        st.subheader("🕸️ Bitcoin Network Topology")
        ...
        st.markdown("### 📊 Network Degree Distribution")
        ...
        st.markdown("### 🔍 Network Characteristics")
    ...
    with tabs[1]:
        st.subheader("💰 Cryptocurrency Economics")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("### 📊 Mining Centralization")
            ...
            centralization_df = pd.DataFrame({
                ...
                'Value': [gini, hhi],
                'Threshold': [0.4, 0.25],
                'Status': ['✅ Acceptable' if gini < 0.4 else '⚠️ High', 
                          '✅ Competitive' if hhi < 0.25 else '⚠️ Concentrated']
            })
        ...
        with col2:
            ...
            network_sizes = list(range(1000, 20000, 1000))
            ...
            st.info("💡 **Insight**: Bitcoin's network value grows quadratically with user adoption")
    
    with tabs[2]:
        st.subheader("🔐 Blockchain Security Assessment")
        ...
        st.markdown("### 🛡️ Security Component Breakdown")
...
# ============================================
...
elif page == "🛡️ BFT Security":
    
    st.header("🛡️ Byzantine Fault Tolerance Analysis")
    ...
    with col3:
        st.metric(
            ...
            delta="✅ Secure" if bft_metrics.get('is_bft_secure', True) else "❌ At Risk"
        )
    
    # BFT Security Status
    st.subheader("🔒 BFT Security Status")
    ...
    st.subheader("🔄 PBFT Consensus Simulation")
    ...
    with col2:
        ...
        if bft_metrics.get('is_bft_secure', True):
            st.success("✅ **System is BFT Secure**")
            st.info(f"🛡️ Can tolerate up to {bft_metrics.get('max_tolerable_bft', 0)} Byzantine nodes")
        else:
            st.error("❌ **Warning: BFT Security Threshold Exceeded**")
            st.warning(f"⚠️ Reduce Byzantine nodes to below {bft_metrics.get('max_tolerable_bft', 0)}")
    
    # BFT Analysis Details
    st.subheader("📋 BFT Analysis Details")
    ...
    bft_analysis_df = pd.DataFrame({
        ...
        'Value': [
            ...
            f"{bft_metrics.get('bft_safety_margin', 0)}",
            "✅ SECURE" if bft_metrics.get('is_bft_secure', True) else "❌ AT RISK"
        ]
    })
    ...
    with st.expander("📚 Understanding Byzantine Fault Tolerance"):
        st.markdown(f"""
        ...
        **Key Principles:**
        ...
        - **BFT Requirement**: `n ≥ 3f + 1` (need at least 3 times the faulty nodes plus 1)
        - **Practical BFT**: `n ≥ 2f + 1` for certain consensus algorithms
        ...
        **In This System:**
        ...
        - Current status: **{"SECURE ✅" if bft_metrics.get('is_bft_secure', True) else "AT RISK ❌"}**
...
# ============================================
...
elif page == "🔮 Transaction Predictor":
    
    st.header("🔮 Real-Time Transaction Risk Predictor")
    ...
    with col1:
        st.subheader("⚙️ Transaction Parameters")
    ...
    with col2:
        ...
        if is_illicit:
            st.error("🚨 **Classification: ILLICIT**")
            st.warning(f"**Confidence**: {risk_score*100:.1f}%")
        else:
            st.success("✅ **Classification: LICIT**")
...
<div style="text-align: center; color: gray; padding: 20px;">
    <p>🔗 <strong>Blockchain Transaction Security Dashboard</strong> v1.0</p>
    <p>Powered by Machine Learning & Advanced Analytics</p>
    <p>Built with Streamlit, Plotly, and scikit-learn</p>
    <p>© 2024 - Elliptic Bitcoin Dataset Analysis</p>
</div>
""", unsafe_allow_html=True)